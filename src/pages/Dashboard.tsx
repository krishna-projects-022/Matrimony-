...
const Dashboard = () => {
  ...
  const [profiles, setProfiles] = useState([
    {
      ...
      image:
        "https://images.unsplash.com/photo-1649972904349-6e44c42644a7?ixlib=rb-4.0.3&auto=format&fit=crop&w=300&q=80",
    },
    {
      id: 2,
      name: "Kavya R.",
    ...
    },
    {
      ...
      education: "B.Com, CA",
      image:
        "https://images.unsplash.com/photo-1472396961693-142e6e269027?ixlib=rb-4.0.3&auto=format&fit=crop&w=300&q=80",
    },
  ]);

  const [interestsSent, setInterestsSent] = useState<number[]>([]);
  const [receivedInterests, setReceivedInterests] = useState<number[]>([]);
  const [showReceived, setShowReceived] = useState(false);

  const handleInterest = (id: number) => {
    setInterestsSent((prev) => [...prev, id]);
    // Simulate receiving interest back for demo purposes
    setReceivedInterests((prev) => [...prev, id]);
  ...
  const displayedProfiles = showReceived
    ? profiles.filter((profile) => receivedInterests.includes(profile.id))
    : profiles;
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8 flex justify-between items-center">
          ...
          >
            ...
            {showReceived ? "Back to Matches" : "Received Interests"}
          </Button>
        </div>
