...
<Card className="shadow-md rounded-2xl">
  <CardHeader className="flex justify-between items-center">
    ...
    <Pencil
      ...
      onClick={() => setEditableSections((prev) => ({ ...prev, horoscope: !prev.horoscope }))}
    />
  </CardHeader>
  <CardContent className="space-y-4 p-6 pt-0 text-sm">
    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
      {editableSections.horoscope ? (
        <>
          <Input value={profile.dob} onChange={(e) => handleInputChange("dob", e.target.value)} placeholder="Date of Birth" />
          <Input value={profile.tob} onChange={(e) => handleInputChange("tob", e.target.value)} placeholder="Time of Birth" />
          <Input value={profile.pob} onChange={(e) => handleInputChange("pob", e.target.value)} placeholder="Birth Place" />
          <Input value={profile.language} onChange={(e) => handleInputChange("language", e.target.value)} placeholder="Language" />
          <Input value={profile.chartStyle} onChange={(e) => handleInputChange("chartStyle", e.target.value)} placeholder="Chart Style" />
        </>
      ) : (
        <>
    ...
    {loading ? (
      ...
      <div className="bg-yellow-400 text-black p-4 rounded-xl flex flex-col items-center justify-center space-y-4 text-center">
        <span className="text-sm md:text-base">Generate your horoscope and get more responses</span>
        <Button
      ...
      <div className="text-center pt-6">
        <Button className="px-6 py-2 text-lg font-semibold" onClick={handleSaveProfile}>
          Save Profile
        </Button>
      </div>
    </div>
